"""Phase 9 pilot tests: spectral_centroid variable-frequency Rust path."""
...
class TestVariableFreqCentroid:
    ...
    def test_variable_freq_matches_python_fallback(self, dtype):
        ...
        with _force_python_spectral_fallback(True):
            expected = librosa.feature.spectral_centroid(S=S, freq=freq)

        actual = librosa.feature.spectral_centroid(S=S, freq=freq)
    ...
    def test_invalid_variable_freq_shape_uses_python_validation(self):
        ...
        with pytest.raises(ParameterError, match="freq.shape mismatch"):
            librosa.feature.spectral_centroid(S=S, freq=bad_freq)
